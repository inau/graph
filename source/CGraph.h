...
class CDirectedGraph : public IGraph<Identifier, Item>
{
protected:
    std::map<Identifier, std::shared_ptr<IVertex<Identifier,Item>>> vertices;
    bool VertexExists(IVertex<Identifier, Item>* pV)
    {
        return(pV != NULL && (vertices.find(pV->GetId()) != vertices.end()) );
    }

public:
    bool AddEdge(IVertex<Identifier, Item>* pFrom, IVertex<Identifier, Item>* pTo)
    {
        bool res = false;
        if(pFrom && pTo && pFrom != pTo)
        {
            if( VertexExists(pFrom) && VertexExists(pTo) )
            {
                pFrom->Connect(pTo);
                res = true;
            }
    ...
    IVertex<Identifier, Item>* GetVertex(Identifier i)
    {
        IVertex<Identifier, Item>* pRes = NULL;
        if( vertices.find(i) != vertices.end() ) pRes = vertices[i].get();
        return pRes;
    };

    IVertex<Identifier, Item>* AddVertex(Identifier i)
    {
        IVertex<Identifier, Item>* pRes = NULL;
        if( vertices.find(i) == vertices.end() ) vertices[i] = std::make_shared<CVertex<Identifier,Item>>(i);
        return pRes;
    };
};

template<class Identifier, class Item>
class CUndirectedGraph : public CDirectedGraph<Identifier, Item>
{
public:
    bool AddEdge(IVertex<Identifier, Item>* pFrom, IVertex<Identifier, Item>* pTo) override
    {
        CDirectedGraph<Identifier,Item>::AddEdge(pFrom,pTo);
        CDirectedGraph<Identifier,Item>::AddEdge(pTo,pFrom);
    };
};
